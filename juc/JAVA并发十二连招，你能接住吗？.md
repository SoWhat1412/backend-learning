...
### 2.2 JDK8
[ConcurrentHashMap](https://mp.weixin.qq.com/s/CjuT8SGKNS4eR_gjhrubGg) 在**JDK8**中抛弃了分段锁，转为用 **CAS** + **synchronized**，同时将**HashEntry**改为**Node**，还加入了**红黑树**的实现，主要还是看**put**的流程(如果看了**扩容**这块，绝对可以好好吹逼一番)。
![在这里插入图片描述](https://img-blog.csdnimg.cn/20201224131113917.png)
...
> 1. 当我们对某个**table[i]**操作时候是用**syn**加速的。
> 2. 取数据的时候用的是**unsafe**硬件级别指令，直接获取指定内存的最新数据。

# 3 、并发基础知识
并发编程的出发点：**充分利用CPU计算资源**，多线程并不是一定比单线程快，要不为什么[Redis](https://mp.weixin.qq.com/s/UfJE6V45MoAQK2RpmNbvhA)6.0版本的核心操作指令仍然是单线程呢？对吧！多线程跟单线程的性能都要具体任务具体分析，**talk is cheap, show me the picture**。
![在这里插入图片描述](https://img-blog.csdnimg.cn/202012251843468.gif#pic_center)
...
### 3.3 线程几个状态
![在这里插入图片描述](https://img-blog.csdnimg.cn/20201225182252803.png)
Java中[线程的状态](https://blog.csdn.net/pange1991/article/details/53860651)分为6种：
1. 初始(New)：
...
>2. 其实线程从[阻塞/等待状态](https://blog.csdn.net/pange1991/article/details/53860651) 到 可运行状态都涉及到**同步队列**跟**等待队列的**，这点在 **[AQS](https://mp.weixin.qq.com/s/bxWgo9IuggDpE1l37JqEhQ)** 有讲。
### 3.4. 阻塞与等待的区别
**阻塞**：
...
### 3.6 wait 跟 sleep 区别
1. 来源不同
> [**wait**](https://www.jianshu.com/p/ffc0c755fd8d) 来自**Object**，**sleep** 来自 **Thread**
2. 是否释放锁
>**wait** 释放锁，**sleep** 不释放
3. 使用范围
...
### 10.1 Synchronized 讲解
 ...
 [Syn](https://mp.weixin.qq.com/s/e_fYFWK5Qnxjmz6Abi7uqw)的底层其实是**C++**代码写的，**JDK6**前是重量级锁，调用的时候涉及到用户态跟内核态的切换，挺耗时的。**JDK6**之前 **Doug Lea**写出了**JUC**包，可以方便的让用于在用户态实现锁的使用，**Syn**的开发者被激发了斗志所以在**JDK6**后对**Syn**进行了各种性能升级。
